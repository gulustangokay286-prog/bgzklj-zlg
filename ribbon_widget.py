...
class RibbonScrollArea(QScrollArea):
    ...
    def set_scroll_enabled(self, enabled: bool):
        ...
        # Çubuk hiç görünmez; dar pencerede kaydırma tekerlek/trackpad ile olur.
        self.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        if not enabled:
...
class RibbonWidget(QWidget):
    ...
    def resizeEvent(self, event):
        ...
        # Yeni düzen ANINDA ve eş zamanlı kurulur. macOS tam ekran geçişi
        # pencerenin anlık görüntüsünü ölçekleyerek oynatır ve Qt'ye tek bir
        # son boyut verir; geciktirmek ya da eski düzeni ortalayıp beklemek
        # (bir önceki deneme) ikonların kaybolup geri gelmesine ve tam ekrandan
        # çıkınca şeridin bir an pencereden taşmasına yol açıyordu.
        w = self.width()

...
def validate(world, rules, positions, bend_rules=False, forced=None, pieces=None):
    """Çizelgeyi sıfırdan denetler.

    bend_rules=True iken yalnızca ARİTMETİĞİN DAYATTIĞI tekrarlar hata
    sayılmaz, ayrı bir listeye ('bent') yazılır: bir dersin (ya da
    öğretmenin) kart sayısı, o kartların sığabildiği gün sayısını aşıyorsa
    "aynı gün tekrar etmesin" hiçbir çizelgede sağlanamaz ve o grupta aynı
    güne düşen kartlar taban olarak kabul edilir. `forced` bu grupların
    kümesidir (problem.impossible_groups); verilmezse burada hesaplanır.

    Bunun dışındaki HER sert kural ihlali, bend_rules ne olursa olsun
    hatadır. Eskiden bayrak bütün sert ihlalleri "esnedi" diye kabul
    ediyordu; CP-SAT'in modellemediği bir kural ihlal edildiğinde bu, ihlalin
    rapora "aritmetik taban" diye yazılıp çizelgenin kullanıcıya gitmesi
    demekti. Kural ekranda açık, çizelgede çiğnenmiş, raporda masum.

    `pieces` — {kart indeksi: [hücre indeksleri]} — BÖLÜNMÜŞ kartlardır:
    2 saatlik blok 1+1 olarak yerleşmiştir. Her parça 1 saatlik bir kart gibi
    denetlenir. Eskiden parçalar hiç denetlenmiyordu: positions'ta -1 olan
    kart atlanıyor, parçaları kapalı saate ya da kural ihlaline düşse bile
    kimse görmüyordu.

    Fiziksel kısıtlar hiçbir bayraktan etkilenmez. Öğretmen çakışması, sınıf
    ...
    if bend_rules and forced is None:
        from .problem import impossible_groups
        forced = impossible_groups(world)
    forced = forced or set()
    pieces = pieces or {}
    class_cells, teacher_cells = {}, {}
    rows=[]
    placed=[]
    for i,(c,idx) in enumerate(zip(world.cards, positions)):
        if idx < 0 and i in pieces and pieces[i]:
            if c.locked_at is not None:
                errors.append(f"Kilitli kart bölünmüş: {' + '.join(c.class_names)} · {c.subject_name}")
            if len(pieces[i]) != c.duration:
    ...
    for c,idx in placed:
        ...
        rows.append((c,d,p))
    for r in rules:
        if r.kind in R.DEFINITION_RULES: continue
        selected=[x for x in rows if r.applies_card(x[0])]
        def hit(detail, bendable=False):
            msg = f"{r.label}: {detail}"
            if not r.is_hard(): soft.append(msg)
            elif bend_rules and bendable: bent.append(msg)
        ...
        if r.kind in R.WINDOW_RULES:
            ...
            for c,d,p in selected:
                if ((r.kind==R.X_MORNING_ONLY and p+c.duration>noon)
                    ...
                    hit(f"{c.subject_name}, gün {d+1}, saat {p+1}")
                         ...
                         R.X_HARD_NOT_ADJACENT,R.X_SUBJECT_NOT_ADJACENT,R.X_MIN_DAYS_BETWEEN):
            for ci,lst in by_class.items():
                for i,(a,ad,ap) in enumerate(lst):
                    for b,bd,bp in lst[:i]:
                        same_day=ad==bd
                        # Ders kimliği AİLE üzerinden: grup tanımıyla Mat1 ile
                        ...
                        bad=((r.kind==R.X_SUBJECT_ONCE_DAY and same_day and same_family)
                            ...
                            or (r.kind==R.X_MIN_DAYS_BETWEEN and same_family and abs(ad-bd)<max(1,r.param)))
                        if bad:
                            # Yalnızca aritmetik taban esner: bu sınıfta bu dersin
                            # (öğretmenin) kartları gün sayısına sığmıyorsa.
                            bendable=False
                            if r.kind in (R.X_SUBJECT_ONCE_DAY,R.X_MIN_DAYS_BETWEEN) and same_family:
                                bendable=('s',ci,a.family) in forced
                            elif r.kind==R.X_TEACHER_ONCE_DAY:
                                bendable=('t',ci,a.teacher) in forced
                            hit(f"{world.classes[ci]}, gün {ad+1}: {a.subject_name} / {b.subject_name}", bendable)
            ...
            for ci,lst in by_class.items():
                ...
                for c,d,p in lst: counts[c.family][d]+=1 if r.kind in (R.X_SUBJECT_MAX_SESSIONS,R.X_EVEN_SPREAD) else c.duration
                for fam,values in counts.items():
                    limit=r.param
                    if r.kind==R.X_EVEN_SPREAD:
                        if max(values)-min(values)>1: hit(f"{world.classes[ci]}, {world.family_name(fam)}: {values}")
                    elif any(v>limit for v in values): hit(f"{world.classes[ci]}, {world.family_name(fam)}: {values}")
            ...
            for c,d,p in selected:
                key=c.family if r.kind==R.Y_SUBJECT_MAX_PARALLEL else c.teacher
